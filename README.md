...
- Copy entries with `gy` then `gp`:
    - `gy` marks the entry under the cursor (or the whole visual selection) and flashes it, like a native yank; a marked directory implicitly covers its children too, because the copy is recursive. `gy` again on the same entry unmarks it.
...
| `FilebufCopyMark` | Flash shown on yanked/pasted lines (links to `Visual`) |
